...
arguments (Input)
  ...
  zEnsembleWindowHalfSize (1, 1) double      % 
  zEnsembleWindowExp2Diameter (1, 1) double  %
  xEnsembleWindowHalfSize (1, 1) double      %
  xEnsembleWindowExp2Diameter (1, 1) double  %
  yEnsembleWindowHalfSize (1, 1) double      %
  yEnsembleWindowExp2Diameter (1, 1) double  %
  displayDaspect (1, 3) double               %
  angioMap (256, 3) double                   %
  saveFolder (1, 1) string                   %
  tauDiff (1, 1) double = 1                  %
...
arguments (Output)
  ...
  if ~(saveFolder == "")
    ...
    save3DMatrixAsTiffRGB(...
      alphaFC2(:, :, :, tauDiff), ...
      fullfile(saveFolder, 'alphaFC2_t' + string(tauDiff) + '_Bscan.tif'), ...
      [0 1], angioMap, ...
      [nZ * displayDaspect(1) / displayDaspect(2), nX, nY], 'linear')
    if verbose
      fprintf('  saved %s \n', fullfile(saveFolder, 'alphaFC2_Bscan.tif'))
    end
    save3DMatrixAsTiffRGB(...
      permute(alphaFC2(:, :, :, tauDiff), [3 2 1]), ...
      fullfile(saveFolder, 'alphaFC2_t' + string(tauDiff) + '_Enface.tif'), ...
      [0 1], angioMap, ...
      [nY * displayDaspect(2) / displayDaspect(3), nX, nZ], 'linear')
    if verbose
      fprintf('  saved %s \n', fullfile(saveFolder, 'alphaFC2_Enface.tif'))
    end
    save3DMatrixAsTiffRGB(...
      alphaFC3(:, :, :, tauDiff), ...
      fullfile(saveFolder, 'alphaFC3_t' + string(tauDiff) + '_Bscan.tif'), ...
      [0 1], angioMap, ...
      [nZ * displayDaspect(1) / displayDaspect(2), nX, nY], 'linear')
    if verbose
      fprintf('  saved %s \n', fullfile(saveFolder, 'alphaFC3_Bscan.tif'))
    end
    save3DMatrixAsTiffRGB(...
      permute(alphaFC3(:, :, :, tauDiff), [3 2 1]), ...
      fullfile(saveFolder, 'alphaFC3_t' + string(tauDiff) + '_Enface.tif'), ...
      [0 1], angioMap, ...
      [nY * displayDaspect(2) / displayDaspect(3), nX, nZ], 'linear')

    if verbose
      fprintf('  saved %s \n', fullfile(saveFolder, 'alphaFC3_Enface.tif'))
